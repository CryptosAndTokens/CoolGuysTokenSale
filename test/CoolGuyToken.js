...
contract("CoolGuyToken", function(accounts){
	...
	it('handles delegated token transfers', function(){
		return CoolGuyToken.deployed().then(function(instance){
			...
			// Try transferring a larger amount than the spenders balance
			return tokenInstance.transferFrom(fromAccount, toAccount, 9999, { from: spendingAccount });
		}).then(assert.fail).catch(function(error){
			assert(error.message.indexOf('revert') >=  0, 'cannont transfer value larger than balance');
			// Try transferring something larger than the approved amount
			return tokenInstance.transferFrom(fromAccount, toAccount, 20, { from: spendingAccount });
		}).then(assert.fail).catch(function(error){
			assert(error.message.indexOf('revert') >= 0, 'transfer value cannot be larger than approved amount');
